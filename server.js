...
function newRoom(code) {
  return {
    code,
    players: [null, null],
    lobby: { picks: [null, null], ready: [false, false], map: 0 },
    game: null,
    emptyTimer: null,
  };
}
...
function resetLobby(room) {
  room.lobby = { picks: [null, null], ready: [false, false], map: room.lobby.map };
  room.game = null;
}
...
wss.on('connection', (ws, req) => {
  ...
  send(ws, { type: 'joined', playerId: slot + 1, room: code });
  broadcast(room, { type: 'lobby', lobby: room.lobby, connected: connCount(room) });
  ...
  ws.on('message', (raw) => {
    ...
    switch (msg.type) {
      ...
      case 'ready':
        ...
        broadcast(room, { type: 'lobby', lobby: room.lobby, connected: connCount(room) });
        if (room.lobby.ready[0] && room.lobby.ready[1] && room.players[0] && room.players[1]) {
          room.game = { turn: 1 };
          broadcast(room, {
            type: 'start',
            map: room.lobby.map,
      ...
      case 'turnDone':
        if (!room.game) break;
        room.game.turn = room.game.turn === 1 ? 2 : 1;
        broadcast(room, { type: 'turn', turn: room.game.turn, wind: randWind(), sync: msg.sync });
        break;
...
server.listen(PORT, () => {
  ...
  console.log('──────────────────────────────────────');
  console.log('  포트리스 미니 서버 시작! (다중 방 지원)');
